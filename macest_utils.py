...
class MacestUtils:
    ...
    def split_data(self, X, y):
        ...
        [X_pp_train, X_conf_train, X_cal, X_test] = map(convert_to_np_array,
                                                        [X_pp_train, X_conf_train, X_cal, X_test])
        return X_pp_train, X_conf_train, X_cal, X_test, y_pp_train, y_conf_train, y_cal, y_test
    ...
    def create_plots(self):
        ...
        plot_data = self.X_test if test_size_condition else self.X_test[0:10**3]
        plot_true = self.y_test if test_size_condition else self.y_test[0:10**3]
        reg_plot.plot_calibration(self.macest_model, plot_data, plot_true, save=True, save_dir=os.path.join(plots_dir, 'plot1'))
...
def convert_to_np_array(data):
    return data.toarray() if type(data) == scipy.sparse.csr.csr_matrix else data
